...
class ContainerManager:
    ...
    @run_command
    def create_container(self, image: str, ports: str, command: str, volumes: str):
        kwargs = {}
        ...
        try:
            ...
            raise ContainerException("Invalid port format.  Must be comma-separated integers.")

        port_mappings = {}
        for port in port_list:
            port_mappings[str(port)] = None
    ...
    def get_container_port(self, container_id: str) -> dict:
        try:
            ports = self.client.containers.get(container_id).ports
            port_mappings = {}
            for container_port, host_ports in ports.items():
                if host_ports:
                    port_mappings[container_port.split('/')[0]] = host_ports[0]["HostPort"]
